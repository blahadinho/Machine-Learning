...
Nval = floor(N/K);
cvhop = Nval;
randomind = idx(randperm(length(idx)));% Select random indices for picking out validation and estimation indices. 
...
for kframe = 1:Nframes % First loop over frames
    ...
    for kfold = 1:K % Then loop over the folds
        ...
        if kfold == K
            valind = randomind(cvlocation+1:end); % Select validation indices
            estind = randomind(1: cvlocation); % Select estimation indices
        else
            valind = randomind(cvlocation+1:cvlocation+cvhop); % Select validation indices
            estind = randomind([1: cvlocation cvlocation+cvhop+1:end]); % Select estimation indices
        end
        ...
        for klam = 1:Nlam  % Finally loop over the lambda grid
            ...
            SEval(kfold,klam) = SEval(kfold,klam) + (1/length(valind))*norm(t(valind)-X(valind,:)*what)^2; % Add validation error at current frame, fold and lambda to the validation error for this fold and lambda, summing tthe error over the frames
            SEest(kfold,klam) = SEest(kfold,klam) + (1/length(estind))*norm(t(estind)-X(estind,:)*what)^2; % Add estimation error at current frame, fold and lambda to the estimation error for this fold and lambda, summing tthe error over the frames
...
[~, lambdaind] = min(MSEval);
lambdaopt = lambdavec(lambdaind); % Select optimal lambda 
