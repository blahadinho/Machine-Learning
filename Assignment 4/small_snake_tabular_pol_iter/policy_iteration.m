...
while 1
    ...
    while 1
        ...
        for state_idx = 1 : nbr_states
            temp = next_state_idxs(state_idx, policy(state_idx));
            if temp > 0
                new_value = rewards.default + gamm*values(temp);
            elseif temp == 0
                new_value = rewards.death;
            else
                new_value = rewards.apple;
            end
            % FILL IN POLICY EVALUATION WITHIN THIS LOOP.
            Delta = max([abs(new_value-values(state_idx)) Delta]);
    ...
    policy_stable = true; 
    for state_idx = 1 : nbr_states
        temp = next_state_idxs(state_idx, :);
        
        q_temp = zeros(1,3);
        for i = 1:nbr_actions
            if temp(i) > 0
                q_temp(i) = rewards.default + gamm*values(temp(i));
            elseif temp(i) == 0
                q_temp(i) = rewards.death;
            else
                q_temp(i) = rewards.apple;
            end
        end
        [~, argmax] = max(q_temp);
        if argmax ~= policy(state_idx)
            policy_stable = false;
        end
        policy(state_idx) = argmax;
