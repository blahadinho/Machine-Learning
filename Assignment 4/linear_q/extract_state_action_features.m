...
for action = 1 : 3 % Evaluate all the different actions (left, forward, right).
    ...
    % [next_next_head_loc, ~] = get_next_info(2, next_move_dir, next_head_loc);
    ...
    % How much will the distance to the apple change after taking action?
    [apple_loc_m, apple_loc_n] = find(grid == -1);
    %dist_to_apple = (abs(apple_loc_m - next_head_loc(1)) + abs(apple_loc_n - next_head_loc(1)))/N;
    %state_action_feats(1, action) = dist_to_apple;
    
    % Will the head hit a wall/body or the apple? (apple:-1,wall:1,nothing:0)
    if grid(next_head_loc(1), next_head_loc(2)) == 1
        state_action_feats(1, action) = 1;
    elseif grid(next_head_loc(1), next_head_loc(2)) == -1
        state_action_feats(1, action) = -1;
    else
        state_action_feats(1, action) = 0;
    end
    
    % Is the head moving towards the apple? (y/n)
    if abs(apple_loc_m - next_head_loc(1)) < abs(apple_loc_m - head_loc(1)) || ...
            abs(apple_loc_n - next_head_loc(2)) < abs(apple_loc_n - head_loc(2))
        state_action_feats(2, action) = 1;
    else
        state_action_feats(2, action) = 0;
    end
    
%     if next_next_head_loc(1) ~= 0 && next_next_head_loc(2) ~= 0 && ...
%             next_next_head_loc(1) ~= 31 && next_next_head_loc(2) ~= 31
%         if grid(next_next_head_loc(1), next_next_head_loc(2)) == 1
...
%         state_action_feats(3, action) = 0;
%     end
    %grid_no_walls = grid(2:end-1, 2:end-1);
%     grid_copy = grid;
%     grid_copy = grid_copy + ismember(grid_copy,-1);
%     grid_copy = abs(grid_copy-1);
%     grid_copy(next_head_loc(1), next_head_loc(2)) = 0;
%     if next_next_head_loc(1) ~= 0 && next_next_head_loc(2) ~= 0
%         grid_copy(next_next_head_loc(1), next_next_head_loc(2)) = 0;
%     end
%     
%     regions = regionprops(bwconncomp(grid_copy,4));
%     %length(regions.Area)
%     nbr_areas = length(regions);
%     if nbr_areas == 1
%         state_action_feats(3, action) = 0;
%     else
%         centroid = regions(2).Centroid;
%         if abs(centroid(2) - next_head_loc(1)) > abs(centroid(2) - head_loc(1)) || ...
%             abs(centroid(1) - next_head_loc(2)) > abs(centroid(1) - head_loc(2))
%             state_action_feats(3, action) = 1;
%         else
%             state_action_feats(3, action) = -1;
%         end
